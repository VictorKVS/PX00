...
**Why:** Synthetic tests prove individual rules, but the current repository may still contain broken references, ID drift, malformed YAML or a Tree_F gap. One integration test must bind the validator to the actual checkout before CI is justified.
...
**Data & processing:** `RepositoryIntegrationTests.test_current_repository_contracts` calls `validate_repository(root)` against the repository root and requires an empty issue list.

**Algorithms / libraries:** No new dependency. Existing `PyYAML==6.0.3` plus Python standard-library `unittest` and `pathlib`.

**DevOps:** The same local unit-test command now also validates the real repository. CI remains deferred until this passes in `G:\1\PX00`.

**Security conclusion:** `PASS_WITH_ACTIONS`. The integration gate strengthens drift detection but does not claim production authorization, tenant isolation, dependency integrity or tamper resistance.

**Tests / evaluation:** Integration test defined; local execution result not yet recorded.

**Next gate:** Execute in the real local clone and preserve PASS/FAIL as separate assurance evidence. Any failure blocks CI and wider runtime work until corrected.
