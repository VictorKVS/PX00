...
from px00.recorder import AppendOnlyEventRecorder, RecorderIntegrityError, TraceKnowledgeContext
...
class ReadOnlyReplayVerifier:
    ...
    def verify(
        ...
        *,
        ...
        events = tuple(events)
        if not events:
            return self._report("INSUFFICIENT_EVIDENCE", request, 0, "NO_MATERIAL_EVENTS", snapshot, knowledge_context)

        if snapshot.run_id != request.run_id:
            return self._report("POLICY_MISMATCH", request, 0, "POLICY_SNAPSHOT_RUN_MISMATCH", snapshot, knowledge_context)
        if authority.run_id != request.run_id or authority.action_request_id != request.action_request_id:
            return self._report("BROKEN_LINEAGE", request, 0, "AUTHORITY_REQUEST_RUN_MISMATCH", snapshot, knowledge_context)
        if authority.policy_snapshot_ref != snapshot.snapshot_id or authority.policy_snapshot_hash != snapshot.snapshot_hash:
            return self._report("POLICY_MISMATCH", request, 0, "AUTHORITY_POLICY_SNAPSHOT_MISMATCH", snapshot, knowledge_context)

        for event in events:
            if event.trace_id != request.trace_id or event.run_id != request.run_id or event.task_id != request.task_id:
                return self._report("BROKEN_LINEAGE", request, 0, "EVENT_CONTEXT_MISMATCH", snapshot, knowledge_context)
            if event.action_request_ref != request.action_request_id:
                return self._report("BROKEN_LINEAGE", request, 0, "EVENT_ACTION_REQUEST_MISMATCH", snapshot, knowledge_context)
            if event.authority_decision_ref != authority.decision_id:
                return self._report("BROKEN_LINEAGE", request, 0, "EVENT_AUTHORITY_MISMATCH", snapshot, knowledge_context)
            if event.policy_snapshot_ref != snapshot.snapshot_id or event.policy_snapshot_hash != snapshot.snapshot_hash:
                return self._report("POLICY_MISMATCH", request, 0, "EVENT_POLICY_SNAPSHOT_MISMATCH", snapshot, knowledge_context)
        ...
        try:
            ...
            return self._report("TAMPER_DETECTED", request, 0, str(exc), snapshot, knowledge_context)

        if persisted.manifest.run_id != request.run_id or persisted.manifest.task_id != request.task_id:
            return self._report("BROKEN_LINEAGE", request, 0, "TRACE_MANIFEST_CONTEXT_MISMATCH", snapshot, knowledge_context)
        if persisted.manifest.event_refs != tuple(event.event_id for event in events):
            return self._report("BROKEN_LINEAGE", request, 0, "TRACE_MANIFEST_EVENT_SET_MISMATCH", snapshot, knowledge_context)
